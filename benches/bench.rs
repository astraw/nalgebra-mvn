...
fn bench_pdf(c: &mut Criterion) {
    ...
    {
        let precision = Matrix2::from_row_slice(
        &[1.0, 0.0,
            0.0, 1.0]);

        c.bench_function("create_from_precision", move |b| b.iter(|| {
            MultivariateNormal::from_mean_and_precision(
                &mu, &precision);
        }));
    }

    let covariance = Matrix2::from_row_slice(
      &[1.0, 0.0,
        0.0, 1.0]);

    c.bench_function("create_from_covariance", move |b| b.iter(|| {
        MultivariateNormal::from_mean_and_covariance(
            &mu, &covariance).unwrap();
    }));
...

    {
        ...
        let xs = MatrixMN::<f64, nalgebra::Dynamic, U2>::from_fn_generic(
            nalgebra::Dynamic::new(100000), U2, |row, col| {
                if col == 0 {
                    row as f64
                } else {
                    1.0
